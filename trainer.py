...
#Varsha edit in order to pass None as a command line parameter
...
def parse_args():
    ...
    parser.add_argument('--measure_2', type=str, default='jaccard', help='measure method. (default: jaccard)')
    parser.add_argument('--dropout', type=float, default=0.2, help='dropout rate (default: 0.2)')
    parser.add_argument('--latent_no_dp', action='store_true', help='no dropout on latent mapping')
    parser.add_argument('--latent_no_xi', action='store_true', help='no xavier init on latent mapping')
    parser.add_argument('--concept_dp', action='store_true', help='use dropout on concept mapping')
    parser.add_argument('--concept_xi', action='store_true', help='use xavier init on concept mapping')
    ...
    # Varsha Start
    parser.add_argument('--exp_name', type=none_or_str, nargs='?', default=None, help='You can pass the name of the experiment')
    parser.add_argument('--save_chkpt_all', action='store_true', help='checkpoints to save (all)')
    parser.add_argument('--use_postag_vocab', type=none_or_str, nargs='?', default=None,
                        help='which postag concept library should be used: WN, treetagger, spacy')
    # Varsha End
...
def main():
    ...
    # Varsha Start (if exp_name is None, the longer path will be used, else the shorter path,
    # if you have multiple runs of exp_name, please define opt.postfix for each run)
    if opt.exp_name is None:
        ...
        mapping_info = "mapping_text_%s_img_%s_tag_vocab_size_%d" % (opt.text_mapping_layers, opt.visual_mapping_layers,
                                                                     opt.tag_vocab_size)
        ...
        opt.logger_name = os.path.join(rootpath, collections_pathname['train'], opt.cv_name, collections_pathname['val'],
                                       model_info, text_encode_info, visual_encode_info, mapping_info, loss_info,
                                       optimizer_info, opt.postfix)  
    else:
        opt.logger_name =  os.path.join(rootpath, collections_pathname['train'], "train_results", opt.exp_name, opt.postfix)
    # Varsha END
    
    if opt.resume:
        log_path = opt.resume.rsplit('/', 1)
        opt.logger_name =  os.path.join(str(log_path[0]))
    ...
    if opt.use_postag_vocab == None:
        ...
        tag_vocab_path = os.path.join(rootpath, collections_pathname['train'], 'TextData', 'tags_POS',
                                      'tags_%s'%opt.use_postag_vocab, 'video_label_th_1')
        tag_path = os.path.join(rootpath, collections_pathname['train'], 'TextData', 'tags_POS',
                                'tags_%s'%opt.use_postag_vocab, 'video_label_th_1.txt')
    ...
    if opt.concate == 'full':
        opt.text_mapping_layers[0] = opt.bow_vocab_size + opt.text_rnn_size*2 \
                                   + opt.text_kernel_num * len(opt.text_kernel_sizes) 
        opt.visual_mapping_layers[0] = opt.visual_feat_dim + opt.visual_rnn_size*2 \
    ...
    # set data loader
    video2frames = {x: read_dict(os.path.join(rootpath, collections_pathname[x], 'FeatureData', opt.visual_feature,
                                              'video2frames.txt')) for x in cap_file}
    data_loaders = data.get_train_data_loaders(caption_files, visual_feats, tag_path, tag_vocab_path, tag_vocab_size,
                                               rnn_vocab, bow2vec, opt.batch_size, opt.workers, video2frames=video2frames)
    val_video_ids_list = data.read_video_ids(caption_files['val'])
    val_vid_data_loader = data.get_vis_data_loader(visual_feats['val'], opt.batch_size, opt.workers, video2frames['val'],
                                                   video_ids=val_video_ids_list)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            no_impr_counter = checkpoint['no_impr_counter']
            lr_counter = checkpoint['lr_counter']
            ...
            validate.validate(opt, tb_logger, val_vid_data_loader, val_text_data_loader, model)
        else:
    ...
    for epoch in range(start_epoch, opt.num_epochs): # bug fixed for resume feature: starting for start_epoch
        ...
        save_checkpoint({
                ...
                'opt': opt,
                'Eiters': model.Eiters,
                'no_impr_counter': no_impr_counter,
                'lr_counter': lr_counter,
            }, is_best, opt.logger_name + '/', epoch, opt.save_chkpt_all)
    ...
    logging.info('best epoch on validation: {}\n'.format(best_epoch))
    ...
    if opt.testCollection == 'iacc.3':
        ...
        striptStr = ''.join(open( 'util/TEMPLATE_do_test.sh').readlines()) #Varsha edit v1
...
# Saving always last and best, and optionally all
def save_checkpoint(state, is_best, prefix, epoch, save_chkpt_all):
    """save checkpoint at specific path"""
    torch.save(state, prefix + 'model_last.pth')
    if is_best:
        shutil.copyfile(prefix + 'model_last.pth', prefix + 'model_best.pth')
    if save_chkpt_all:
        shutil.copyfile(prefix + 'model_last.pth', prefix + 'checkpoint_epoch_%s.pth'%epoch)
