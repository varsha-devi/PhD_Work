...
def cosine_sim(im, s):
    ...
    """
    return l2norm(im).mm(l2norm(s).t())
...
def jaccard_sim_transform(im, s, test_opt):
    """ Jaccard similarity with sigmoid included.
    """
    if test_opt is None:
        scale, shift, power, topk = 1.0, 0.0, 1.0, s.size(1)
    else:
        scale, shift, power, topk = test_opt.scale, test_opt.shift, test_opt.power, test_opt.topk
    im_bs = im.size(0)
    s_bs = s.size(0)
    im = torch.pow(torch.sigmoid(scale*torch.sub(im, shift)), power).unsqueeze(1).expand(-1,s_bs,-1)
    s = torch.pow(torch.sigmoid(scale*torch.sub(s, shift)), power).unsqueeze(0).expand(im_bs,-1,-1)
    ...
    if topk == im.size(2):
        ...
        k = topk
        min_val = torch.min(im,s)
        max_val = torch.max(im,s)

        sort_min_val, idx_min = torch.sort(min_val, descending=True) #(1000, 2990, 512)
        topk_min_val = sort_min_val[:, :, :k]
        topk_min_idx = idx_min[:, :, :k]
        
        topk_max_val = torch.gather(max_val, 2, topk_min_idx)

        intersection_topk = topk_min_val.sum(-1)
        union_topk = topk_max_val.sum(-1)
        score = intersection_topk / union_topk
...
def jaccard_sim(im, s):
    ...
    im_bs = im.size(0)
    s_bs = s.size(0)
    im = torch.sigmoid(im).unsqueeze(1).expand(-1,s_bs,-1)
    s = torch.sigmoid(s).unsqueeze(0).expand(im_bs,-1,-1)
    intersection = torch.min(im,s).sum(-1)
    union = torch.max(im,s).sum(-1)
    score = intersection / union
    return score
